...
import { ReduxAction, StoreState } from './interfaces';

function requestStatus(state = {}, action: ReduxAction) {
...
function books(state : Book[] = [], action: ReduxAction) {
    switch (action.type) {
...
function lastUpdateTime(state = new Date(), action: ReduxAction) {
    switch (action.type) {
 ...
 */
function booksStoreRootReducer(
